...
class MMoEDeepLabv3(MTLDeepLabv3):
    def __init__(self, tasks, num_experts, input_size):
        super().__init__(tasks)
        self.tasks = tasks
        self.num_experts = num_experts
        self.input_size = input_size
        self.input_flatten_dim = np.array(input_size, dtype=int).prod()
        ch = [256, 512, 1024, 2048]
        self.experts_shared = nn.ModuleList([self.build_encoder() for _ in range(self.num_experts)])
        self.gate_specific = nn.ModuleDict({t: nn.Sequential(nn.Linear(self.input_flatten_dim, self.num_experts),
                                                                nn.Softmax(dim=-1)) for t in self.tasks})
    ...
    def forward(self, x):
        ...
        out = [0 for _ in self.tasks]
        for i, t in enumerate(self.tasks):
            selector = self.gate_specific[t](torch.flatten(x, start_dim=1))
            gate_rep = torch.einsum('ij..., ji -> j...', experts_shared_rep, selector)
            out[i] = F.interpolate(self.decoders[i](gate_rep), size=[im_h, im_w], mode='bilinear', align_corners=True)
            if t == 'normal':
                out[i] = out[i] / torch.norm(out[i], p=2, dim=1, keepdim=True)
        return out
    
    def build_encoder(self):
        # TODO move to MTLDeepLabv3 as a shared function and reimplement MTLDeepLabv3 __init__
        backbone = ResnetDilated(resnet.resnet50())
        shared_conv = nn.Sequential(backbone.conv1, backbone.bn1, backbone.relu1, backbone.maxpool)
        shared_layer1 = backbone.layer1
        shared_layer2 = backbone.layer2
        shared_layer3 = backbone.layer3
        shared_layer4 = backbone.layer4
        return nn.Sequential(
            shared_conv, 
            shared_layer1,
            shared_layer2,
